...
class SongViewModel @Inject constructor(
    ...
    private val _uploaderSongsOther = MutableLiveData<List<SongModels>?>()
    val uploaderSongsOther: LiveData<List<SongModels>?> get() = _uploaderSongsOther
...
//            _message.value = null
    ...
    fun getUserAndUploadedSongsOther(userId: String) {
        _loading.value = true
        viewModelScope.launch {
            _loading.value = true
            try {
                _uploaderSongsOther.value = songRepository.getSongsByUserUploadedOther(userId)
            } catch (e: Exception) {
                Log.e("SongViewModel", "Error fetching data: ${e.message}")
                _uploaderSongsOther.postValue(emptyList())
            }
            _uploaderSongsOther.value =null
            _loading.value =false
        }
    }
    ...
    private fun normalizeString(input: String): String {
        // Loại bỏ dấu thanh (diacritics) bằng Normalizer hà Nội -> ha noi
                ...
                val filteredPlaylists = allPlaylists.filter { playlist ->
                    ...
                    val matchSongTitles = playlist.songIds?.any { songId ->
                        songMap[songId]?.let { song ->
                            normalizeString(song.title).contains(searchQuery) || normalizeString(song.artist).contains(searchQuery)
                        } ?: false
                    } ?: false
