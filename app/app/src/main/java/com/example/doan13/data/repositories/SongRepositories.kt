...
interface SongRepositories{
    ...
    suspend fun getSongsByUserUploadedOther(userId: String): List<SongModels>
...
//            / Tải thumbnail lên Cloudinary từ 25% cho ảnh
            ...
            // Tải MP3 lên Cloudinary phần còn lại
...
//lấy ra những bài hát mới nhất
    ...
    override suspend fun getSongsByUserUploadedOther(userId: String): List<SongModels> {
        return try {
            val userDoc = db.collection("users").document(userId).get().await()
            val user = userDoc.toObject(UserModel::class.java)
            val songUploadedIds = user?.uploadedSongs ?: emptyList()
